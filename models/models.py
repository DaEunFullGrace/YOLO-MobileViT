...
def create_modules(module_defs, img_size, cfg):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        elif mdef['type'] == 'vitblock':  # nn.Sequential() placeholder for 'shortcut' layer
            depth = mdef['depth']
            kernel = mdef['size']
            filters = mdef['filters']
            d_model = mdef['d_model']
            num_heads = mdef['num_heads']
            patch_size = mdef['patch_size']
            
            modules.add_module('ViTBlock', ViTBlock(in_chan=filters,
                                                ker_size=kernel,
                                                d_model=d_model,
                                                num_heads=num_heads,
...
class ViTBlock(nn.Module):
    def __init__(self, in_chan, d_model, num_heads, 
                 ker_size=3, dim_feedforward=None, patch_size=8, 
                 trans_depth=2, no_fuse=False, dropout=0.1):
        ...
        self.n_trans = trans_depth
        self.trans_depth = trans_depth
        self.trans_dim = d_model
        
        self.conv_3x3_in = Conv3x3Layer(in_chan=in_chan, out_chan=in_chan, ker_size=ker_size, stride=1)
        # projection
        self.conv_1x1_in = Conv1x1Layer(in_chan=in_chan, out_chan=self.trans_dim, ker_size=1, stride=1)
        self.conv_1x1_out = Conv1x1Layer(in_chan=self.trans_dim, out_chan=in_chan, ker_size=1, stride=1)
        # fusion
        self.conv_3x3_out = Conv3x3Layer(in_chan=2*in_chan, out_chan=in_chan, ker_size=ker_size, stride=1)

        self.transformer = TransformerEncoder(num_layers=self.trans_depth, 
                                              d_model=d_model,
    ...
    def unfolding(self, feature_map: Tensor) -> Tuple[Tensor, Dict]:
        ...
        reshaped_fm = transposed_fm.reshape(batch_size, in_channels, num_patches, self.patch_w * self.patch_h)
        # [B, C, N, P] --> [B, P, N, C]
    ...
    def folding(self, patches: Tensor, info_dict: Dict) -> Tensor:
        ...
        # [BP, N, C] --> [B, P, N, C]
        patches = patches.contiguous().view(info_dict["batch_size"], 
                                            self.patch_w * self.patch_h, info_dict["total_patches"], -1)
    ...
    def forward(self, x):
        ...
        # 2) Transfotmer as convolution
        # unfold
#         x = rearrange(x, 'b d (h ph) (w pw) -> b (ph pw) (h w) d', ph=self.patch_h, pw=self.patch_w)
        x, info_dict = self.unfolding(x)
        # transformer
        x, attn_score = self.transformer(x)
        # fold
#         x = rearrange(x, 'b (ph pw) (h w) d -> b d (h ph) (w pw)', 
#                       h=h//self.patch_h, w=w//self.patch_w, ph=self.patch_h, pw=self.patch_w)
        x = self.folding(x, info_dict)
        
        # 3) fusion
        x = self.conv_1x1_out(x)
        x = self.conv_3x3_out(torch.cat((residual,x), dim=1))
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), verbose=False):
        ...
        self.scale_f = [1.0, 1.6]
        # self.ms_infer = ms_infer
    ...
    def forward(self, x, augment=False, verbose=False, ms_infer=False, scale_f=1.0):
        if not augment and not ms_infer:
            return self.forward_once(x, verbose=verbose)
        elif augment and not ms_infer:  # Augment images (inference and test only) https://github.com/ultralytics/yolov3/issues/931
            ...
            y = torch.cat(y, 1)
            return y, None
        elif not augment and ms_infer:
            y = []
            s = [1.0]
            s.append(scale_f)

            for i, xi in enumerate((x,
                                    torch_utils.scale_img(x, 
                                                        ratio=scale_f, 
                                                        same_shape=False),
                                    )):
                # print("---------------------------")
                # print(i, xi.shape)
                # print("scale factor", s[i])
                y.append(self.forward_once(xi,
                                            verbose=verbose, 
                                            ms_infer=ms_infer, 
                                            scale_f=s[i])[0])
    ...
    def forward_once(self, x, augment=False, verbose=False, ms_infer=False, scale_f=1.0):
        img_size = x.shape[-2:]  # height, width
        yolo_out, out = [], []
        if verbose:
            print("-----------------------------------------------")
...
def load_darknet_weights(self, weights, cutoff=-1):
    ...
    for i, (mdef, module) in enumerate(zip(self.module_defs[:cutoff], self.module_list[:cutoff])):
        if mdef['type'] == 'convolutional':
            ...
            ptr += nw
        elif mdef['type'] == 'vitblock':
            vitblock = module[0]
            nb = vitblock.bias.numel()
            vit_b = torch.from_numpy(weights[ptr:ptr + nb]).view_as(vitblock.bias)
            vitblock.bias.data.copy_(vit_b)
            ptr += nb
            nw = vitblock.weight.numel()
            vitblock.weight.data.copy_(torch.from_numpy(weights[ptr:ptr + nw]).view_as(vitblock.weight))
            ptr += nw
    ...
    with open(path, 'wb') as f:
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs[:cutoff], self.module_list[:cutoff])):
            if mdef['type'] == 'convolutional':
                ...
                vitblock = module[0]
                vitblock.bias.data.cpu().numpy().tofile(f)
                vitblock.weight.data.cpu().numpy().tofile(f)
